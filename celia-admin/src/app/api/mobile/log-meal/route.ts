...
export async function POST(req: NextRequest) {
  try {
    ...
    const body = (await req.json()) as LogMealBody;
    const items = normalizeItems(body.items);
    if (items.length === 0) {
      return NextResponse.json({ error: 'No food items to log' }, { status: 400 });
    }

    const warnings = Array.isArray(body.warnings) ? body.warnings.map(String).filter(Boolean) : [];
    const loggedAtText = typeof body.loggedAt === 'string' ? body.loggedAt : '';
    const loggedAt = Number.isNaN(Date.parse(loggedAtText)) ? new Date().toISOString() : loggedAtText;
    ...
    const payload = {
      ...
      confidence: Math.min(1, safeNumber(body.confidence, 0)),
    ...
    const { data, error } = await supabase.from('user_meals').insert(payload).select().single();
    if (error) {
