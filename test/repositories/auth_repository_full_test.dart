...
void main() {
  ...
  test(
    'AuthRepository constructor can use default factories (test override)',
    () {
  ...
  test(
    'DefaultAppleSignInClient delegates to plugin (throws in test env)',
    () async {
  ...
  test('signIn success and null-user failure path', () async {
    ...
    when(
      () => auth.signInWithEmailAndPassword(email: 'e2', password: 'p2'),
    ).thenAnswer((_) async => cred2);
    expect(() => repo.signIn('e2', 'p2'), throwsException);
  });

  test(
    'signUp success (send verification ok + send verification throws)',
    () async {
      final auth = MockFirebaseAuth();
      final cred = MockUserCredential();
      final user = MockUser();
  ...
  test(
    'signUp failure when result.user is null and when createUser throws',
    () async {
      final auth = MockFirebaseAuth();
      final cred = MockUserCredential();
      ...
      when(
        () => auth.createUserWithEmailAndPassword(email: 'e2', password: 'p2'),
      ).thenThrow(Exception('nope'));
  ...
  test(
    'signInWithApple Android path covers nonce + oauth + success + missing args',
    () async {
      final auth = MockFirebaseAuth();
      final platform = MockPlatformInfo();
      ...
      when(
        () => auth.signInWithCredential(any()),
      ).thenAnswer((_) async => userCredNull);
