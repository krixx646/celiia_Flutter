...
class MockUsersCollection extends Mock
    implements CollectionReference<Map<String, dynamic>> {}

class MockUserDoc extends Mock
    implements DocumentReference<Map<String, dynamic>> {}

class MockUserSnap extends Mock
    implements DocumentSnapshot<Map<String, dynamic>> {}

class MockConversationsCollection extends Mock
    implements CollectionReference<Map<String, dynamic>> {}

class MockConversationDoc extends Mock
    implements DocumentReference<Map<String, dynamic>> {}
...
void main() {
  ...
  test(
    'ensureUserDocument creates doc when missing (saveConversation)',
    () async {
  ...
  test(
    'getConversations fallback path when orderBy throws FirebaseException',
    () async {
      final auth = MockFirebaseAuth();
      final user = MockUser();
      when(() => user.uid).thenReturn('u1');
      ...
      when(() => userDoc.collection('conversations')).thenReturn(convs);
      when(
        () => convs.orderBy('timestamp', descending: true),
      ).thenReturn(query);
  ...
  test(
    'deleteConversation does not retry on non-retryable FirebaseException',
    () async {
      final auth = MockFirebaseAuth();
      final user = MockUser();
      when(() => user.uid).thenReturn('u1');
  ...
  test(
    'ensureUserDocument throws if auth becomes null between calls',
    () async {
      final auth = MockFirebaseAuth();
      final user = MockUser();
      when(() => user.uid).thenReturn('u1');
