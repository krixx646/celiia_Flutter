...
void main() {
  ...
  test('ChatRepository.createUser throws when API response contains error code/message', () async {
  ...
  test('ChatRepository.getMessages dedupes options and sets user/bot ids', () async {
    final api = MockBotpressApi();
  ...
  test('ChatRepository.createUser success + parse failures + api exception', () async {
    final api = MockBotpressApi();
    ...
    when(() => api.createUser(any())).thenAnswer((_) async => BotpressUserResponse(key: null, user: null));
    expect(() => repo.createUser(), throwsException);
  ...
  test('ChatRepository.createConversation success + error wrap', () async {
    ...
    when(() => api.createConversation(any(), any())).thenAnswer(
      (_) async => BotpressConversationResponse(
        conversation: BotpressConversation(id: 'c', createdAt: 't1', updatedAt: 't2'),
      ),
    );
    final repo = ChatRepository(api: api);
    ...
    when(() => api.createConversation(any(), any())).thenThrow(Exception('nope'));
    expect(() => repo.createConversation('uk'), throwsException);
  });

  test('ChatRepository.sendMessage uses text or image endpoint and wraps errors', () async {
    final api = MockBotpressApi();
    ...
    final m2 = await repo.sendMessage('uk', 'cid', 'https://example.test/x.png');
    expect(m2.type, 'image');

    when(() => api.sendMessage(any(), any(), any())).thenThrow(Exception('nope'));
    expect(() => repo.sendMessage('uk', 'cid', 'hello'), throwsException);
  });

  test('ChatRepository.getMessages maps message types and wraps errors', () async {
    final api = MockBotpressApi();
    when(() => api.getMessages('uk', 'cid')).thenAnswer((_) async {
      return BotpressMessagesResponse(
        messages: [
          ...
          BotpressMessage(
            ...
            conversationId: 'cid',
            userId: 'botpress',
            payload: MessagePayload(type: 'text', options: [MessageOption(label: 'A', value: 'a')]),
            createdAt: 't',
          ),
          BotpressMessage(
            id: 't',
