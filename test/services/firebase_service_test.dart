...
void main() {
  ...
  test('FirebaseService.initialize iOS fallback throws when env missing', () async {
    // Save originals
    ...
    try {
      ...
      FirebaseService.firebaseInitializeApp = ({FirebaseOptions? options}) async {
        // Force the fallback path
        throw Exception('boom');
      };
      FirebaseService.googleSignInInitialize = () async {};
      FirebaseService.appCheckActivate = ({required androidProvider, required appleProvider}) async {};

      await expectLater(FirebaseService.initialize(), throwsA(isA<FirebaseException>()));
    } finally {
  ...
  test('FirebaseService.initialize iOS fallback succeeds when env provided', () async {
    // Save originals
    final origIsIOS = FirebaseService.isIOS;
    final origEnv = FirebaseService.env;
    final origInit = FirebaseService.firebaseInitializeApp;
